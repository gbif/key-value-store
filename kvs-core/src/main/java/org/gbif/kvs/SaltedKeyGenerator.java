...
/**
 * Utility class to generate salted keys based on a number of buckets/splits.
 * Computed keys are padded with an integer value.
 */
public class SaltedKeyGenerator implements Serializable {
    ...
    /**
     *  Computes a salted key based on a expected number of buckets.
     *  The produced key is padded with zeros to the left + logicalKey.hasCode*numOfBuckets + logicalKey.
    ...
    public byte[] computeKey(String logicalKey) {
        return (String.format(paddingFormat, logicalKey.hashCode() % numOfBuckets) +  logicalKey ).getBytes();
    }


    /**
     *  Computes a salted key based on a expected number of buckets.
     *  Produces a key padded with zeros to the left + new String(logicalKey).hasCode*numOfBuckets + new String(logicalKey).
     * @param logicalKey logical identifier
     * @return a zeros left-padded string {0*}+bucketNumber+logicalKey
     */
