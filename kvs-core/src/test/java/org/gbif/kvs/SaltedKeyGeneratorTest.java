...
public class SaltedKeyGeneratorTest {
    ...
    public void saltedKeyGenerationTest() {
        ...
        Assert.assertTrue("Salted key must end with provided logical key", saltedKey.endsWith(TEST_LOGICAL_KEY));

        String bucket = new String(SALT_KEY_GENERATOR.bucketOf(saltedKey));

        Assert.assertTrue("Salted key must end with provided logical key", saltedKey.startsWith(bucket));
    }
    ...
    public void evenlyDistributedKeys() {
       ...
       Map<String,Long> counts = IntStream.rangeClosed(1, 16)
                                    .mapToObj(key -> SALT_KEY_GENERATOR.computeKey(Integer.toString(key)))
                                    .collect(Collectors.groupingBy(key -> new String(SALT_KEY_GENERATOR.bucketOf(key)),
                                                                   Collectors.counting()));
       ...
       counts.values().stream().mapToDouble(count -> count).average().ifPresent(average ->
                Assert.assertEquals(average, NUM_OF_BUCKETS,0.0001)
        );
    }
}
